...
try {
    $stmt = $conn->prepare(
        "SELECT e.*, u.full_name AS intern_name, u.email AS intern_email,
                CASE WHEN s.full_name IS NULL OR s.full_name = '' THEN NULL ELSE s.full_name END AS supervisor_name
         FROM evaluations e
         INNER JOIN users u ON e.intern_id = u.id
         LEFT JOIN users s ON e.supervisor_id = s.id
         WHERE e.id = ? AND e.supervisor_id = ? LIMIT 1"
    );
    $stmt->execute([$id, (int) ($_SESSION['user']['id'] ?? 0)]);
    $ev = $stmt->fetch();
} catch (PDOException $e) {
    error_log('View evaluation fetch error: ' . $e->getMessage());
    $ev = false;
}

// If not found, show a friendly message
if (!$ev) {
    // Show minimal page indicating not found
    ?><!doctype html>
    <html>
    <head>
        <meta charset="utf-8">
...
// Fallback supervisor name: if the DB join returned NULL (e.g., hard-coded supervisor accounts), use session name
$supervisorName = $ev['supervisor_name'] ?? null;
...
<div class="page">
    ...
    <div class="card card-inner">
        ...
        <div class="details">
            ...
            <div class="value"><?php echo htmlspecialchars($supervisorName); ?></div>
